...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ePay
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ePay
    // Mac: ~/Library/Application Support/ePay
    // Unix: ~/.epay
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ePay";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "epay.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No epay.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override epay.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "epayd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong ePay will not work properly.");
