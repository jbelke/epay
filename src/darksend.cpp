...
/* *** BEGIN DARKSEND MAGIC - EPAY **********
    Copyright 2014, ePay Developers
        eduffield - evan@epay.io
*/
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeePayAmount*COIN - pwalletMain->GetAnonymizedBalance();
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeePayAmount = (rand() % nLeftToAnon)+3;
                } else {
